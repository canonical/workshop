```{eval-rst}
.. meta::
   :description: Release notes for Workshop 0.9.3 and SDKcraft 0.9.2, covering
                 the reworked boot process that prepares for VM support and
                 persists the machine ID and SSH host keys, per-workshop *.wp
                 hostnames with a simplified hostname-fallback note, new
                 productid and vendorid attributes for the custom-device
                 interface, smoother onboarding when LXD is missing or outdated,
                 storage pool monitoring, the sdkcraft whoami token location,
                 reference SDKs published under Canonical, and v1 of the
                 Workshop GitHub Action.
```

# Workshop 0.9.3 release notes

## 6 July 2026

These release notes cover new features and changes in Workshop 0.9.3 and
SDKcraft 0.9.2.
...
Workshop 0.9.3 and SDKcraft 0.9.2 rework the workshop boot process in
preparation for VM support, give every workshop a `*.wp` hostname, and add
`productid` and `vendorid` attributes to the `custom-device` interface. The
release also improves onboarding when LXD is missing or outdated, adds storage
pool monitoring, reports where `sdkcraft whoami` stores its token, and ships v1
of the Workshop GitHub Action.
...
- SDKcraft: https://github.com/canonical/sdkcraft/compare/0.9.1...0.9.2
