```{eval-rst}
.. meta::
   :description: Release notes for Workshop 0.9.4, covering a fix for a security
                 issue when connecting to the host's Unix socket through a
                 tunnel, ID-mapped SDK volumes that fix sharing SDKs between
                 users and between containers and VMs, a new system service that
                 reports workshop readiness to LXD ahead of VM support, new
                 Antigravity CLI, openjdk, maven, and gradle reference SDKs, and
                 documentation improvements covering contributing, SDK content
                 sharing, workshop hostnames, and the SDK Store.
```
...
Workshop 0.9.4 fixes a security issue when connecting to the host's Unix socket
through a tunnel, moves SDK volumes to ID-mapped mounts to fix sharing SDKs
between users and between containers and VMs, and adds a system service that
reports workshop readiness to LXD in preparation for VM support.
