```{eval-rst}
.. meta::
   :description: Release notes for Workshop 0.9.1 and SDKcraft 0.9.1, covering
                 the custom-device interface, unused SDK snapshot cleanup,
                 clearer exec and refresh output, SDK Store pre-handshake
                 retries, the website field in sdk info, the new sdkcraft
                 revisions command, and ubuntu@26.04 base support.
```

# Workshop 0.9.1 release notes

## 8 June 2026

These release notes cover new features and changes in Workshop 0.9.1 and
SDKcraft 0.9.1.
...
Workshop 0.9.1 and SDKcraft 0.9.1 add a `custom-device` interface, the new
