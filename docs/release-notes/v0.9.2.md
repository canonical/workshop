```{eval-rst}
.. meta::
   :description: Release notes for Workshop 0.9.2, covering stricter validation
                 that rejects unknown fields in user-facing YAML, friendly host
                 names through systemd-resolved, clearer --wait-on-error
                 messages, device group fixes that restore device access across
                 bases, and the website property for reference SDKs.
```

# Workshop 0.9.2 release notes

## 22 June 2026

These release notes cover new features and changes in Workshop 0.9.2.
...
Workshop 0.9.2 adds stricter validation for user-facing YAML files, friendly
host names for workshops, and clearer error guidance, alongside a
