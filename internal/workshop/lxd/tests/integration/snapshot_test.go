...
//go:embed snapshot-format-container.yaml
var snapshotFormatContainer []byte

//go:embed snapshot-format-vm.yaml
...
func (s *snapshotSuite) testSnapshotFormat(c *check.C, confinement workshop.Confinement, formatYAML []byte) {
	...
	c.Assert(err, check.IsNil)
	defer func() {
		reverr := s.bd.StopWorkshop(s.ctx, wf.Name, false)
		c.Check(reverr, check.IsNil)
	}()
...
func (s *snapshotSuite) workshopFormat(c *check.C, file *workshop.File, snapshot workshop.Snapshot) api.InstancePut {
	...
	// Checked in the LXD backend unit tests, where it can be updated easily.
	var config map[string]any
	c.Assert(yaml.Unmarshal([]byte(inst.Config["cloud-init.user-data"]), &config), check.IsNil)
	delete(inst.Config, "cloud-init.user-data")
...
func (s *snapshotSuite) snapshotDiff(c *check.C, base string, confinement workshop.Confinement) {
	...
	cloneSnapshot.Sdks = append(slices.Clone(cloneSnapshot.Sdks), sdk.ContentID{
		Name:     "test-sdk",
	...
	c.Assert(err, check.IsNil)
	defer func() {
		err1 := s.bd.StopWorkshop(s.ctx, "test", false)
