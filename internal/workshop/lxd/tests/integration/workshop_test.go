...
func (f *wsOps) TestLxdBackendWorkshopStashUnstash(c *check.C) {
	...
	image, err := f.bd.GetBase(f.ctx, wf.Base, workshop.RuntimeLXDContainer)
	c.Assert(err, check.IsNil)
	err = f.bd.DownloadBase(f.ctx, image, nil)
	c.Assert(err, check.IsNil)
	snapshot := workshop.BaseOnly(f.bd.FormatRevision(), image.Name, workshop.RuntimeLXDContainer, image.Fingerprint)
...
func (f *wsOps) TestLxdBackendDownloadBase(c *check.C) {
	...
	image, err := f.bd.GetBase(f.ctx, "ubuntu@22.04", workshop.RuntimeLXDContainer)
	c.Assert(err, check.IsNil)
	c.Check(image.Name, check.Equals, "ubuntu@22.04")
	c.Check(image.Runtime, check.Equals, workshop.RuntimeLXDContainer)
...
func (f *wsOps) TestLxdBackendGetOrDownloadMalformedBase(c *check.C) {
	image := workshop.BaseImage{Name: "ubuntu:24.04", Runtime: workshop.RuntimeLXDContainer, Fingerprint: ""}
	_, err := f.bd.GetBase(f.ctx, image.Name, workshop.RuntimeLXDContainer)
...
func (f *wsOps) TestLxdBackendDownloadBaseImageNotFound(c *check.C) {
	_, err := f.bd.GetBase(f.ctx, "ubuntu@1.01", workshop.RuntimeLXDContainer)
	c.Check(err, check.ErrorMatches, `base "ubuntu@1.01" not found.*`)

	image := workshop.BaseImage{Name: "ubuntu@22.04", Runtime: workshop.RuntimeLXDContainer, Fingerprint: "##################"}
	err = f.bd.DownloadBase(f.ctx, image, nil)
...
func (f *wsOps) TestLxdBackendDownloadMultipleBasesConcurrently(c *check.C) {
	...
	for i, b := range workshop.SupportedBases {
		wg.Go(func() {
			image, err := f.bd.GetBase(f.ctx, b, workshop.RuntimeLXDContainer)
			c.Assert(err, check.IsNil)
...
func (f *wsOps) TestLxdBackendReuseCachedBase(c *check.C) {
	...
	image := workshop.BaseImage{Name: "ubuntu@22.04", Runtime: workshop.RuntimeLXDContainer, Fingerprint: imageCached.Fingerprint}
	err := f.bd.DownloadBase(f.ctx, image, nil)
	c.Assert(err, check.IsNil)
...
func (f *wsOps) TestLxdBackendWorkshopLaunch(c *check.C) {
	image, err := f.bd.GetBase(f.ctx, "ubuntu@24.04", workshop.RuntimeLXDContainer)
	c.Assert(err, check.IsNil)
	err = f.bd.DownloadBase(f.ctx, image, nil)
	c.Assert(err, check.IsNil)
