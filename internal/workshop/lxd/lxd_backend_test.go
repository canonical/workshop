...
func (f *LxdBeTests) TestDefaultContainerConfig(c *check.C) {
	...
	c.Check(hex.EncodeToString(digest[:]), check.Equals, "6549a8fcf46dc87915d2372cfe23f811565065130d60d70f3a7b6ede739b87aa9a1c6d744492cbdaca584c4e928092f5")
}
...
func (f *LxdBeTests) TestDefaultVMConfig(c *check.C) {
	...
	// downloads a new base image or system SDK.
	digest := sha3.Sum384([]byte(cfg["cloud-init.user-data"]))
	c.Check(hex.EncodeToString(digest[:]), check.Equals, "ccf2446cfb121f945e063a9cbdf75e0084701ba58b8291becea1ca5abe0f4bfd5ab9bfa4cdf3a88eb19e85ed3580ee76")
}
