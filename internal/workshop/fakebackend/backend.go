...
type FakeWorkshopBackend struct {
	...
	baseLock             sync.Mutex
	GetBaseCallback      func(ctx context.Context, base string, runtime workshop.Runtime) (workshop.BaseImage, error)
...
func (b *FakeWorkshopBackend) GetBase(ctx context.Context, base string, runtime workshop.Runtime) (workshop.BaseImage, error) {
	b.baseLock.Lock()
	defer b.baseLock.Unlock()

	if b.GetBaseCallback != nil {
		return b.GetBaseCallback(ctx, base, runtime)
	}
	return workshop.BaseImage{Name: base, Runtime: runtime, Fingerprint: "fakeimage123"}, nil
}
