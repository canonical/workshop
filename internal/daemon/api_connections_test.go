...
func (s *apiSuite) TestConnectFailureOnConfinement(c *check.C) {
	...
	defer testutil.FakeFunc(supportsPlugCallback, &s.secBackend.SupportsPlugCallback)()
	...
	err = chg.Err()
	st.Unlock()
	c.Check(err, check.ErrorMatches, `(?s).*\(test devices are only available to containers\).*`)
